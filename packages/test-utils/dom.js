...
/**
 * Find the HTMLButtonElement that includes the given `text`
 *
 * @param {HTMLElement} node
 * @param {string} text
 * @returns {?HTMLButtonElement}
 */
export function getButtonByText(node, text) {
  if (node.tagName === 'BUTTON') {
    const match = getByText(node, text);
    if (match) {
      return node;
    }
  }
  ...
  for (const child of node.childNodes) {
    const match = getButtonByText(child, text);
    if (match) {
      return match;
    }
  }
